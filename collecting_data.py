        ...
        for event in pygame.event.get():
            if event.type == pygame.KEYDOWN:
                ...
                if key_input[pygame.K_r]:
                    ...
                    pygame.display.quit()
                    break
        if(take_picture1 == True and detect):
            frames += 1
            print "Frame number : ", frames
            training_image = np.vstack((training_image, unroll))
            output_array = np.vstack((output_array, true_label[0]))
            cv2.imwrite('training_images/fenil/training_image%d.jpg'%frames, test)
            if(frames>=50):
                take_picture1=False;
            frame=0;
        elif(take_picture2 == True and detect):
            frames += 1
            print "Frame number : ", frames
            training_image = np.vstack((training_image, unroll))
            ...
            if (frames >= 50):
                take_picture2 = False;
            frame=0;
        elif(take_picture3 == True and detect):
            frames += 1
            print "Frame number : ", frames
            training_image = np.vstack((training_image, unroll))
            output_array = np.vstack((output_array, true_label[2]))
            cv2.imwrite('training_images/mam/training_image%d.jpg'%frames, test)
            if (frames >= 50):
                take_picture3 = False;
            frame=0;
        elif (take_picture4 == True and detect):
            frames += 1
            print "Frame number : ", frames
            training_image = np.vstack((training_image, unroll))
            output_array = np.vstack((output_array, true_label[3]))
            cv2.imwrite('training_images/sir/training_image%d.jpg' % frames, test)
            if (frames >= 50):
                take_picture4 = False;
            frame=0;
